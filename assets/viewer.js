...
const Viewer = (() => {
  ...
  async function init() {
    ...
    // Load Giscus reactions/comments for published stories
    const gIssueId = qp("id");
    if (gIssueId) loadGiscus(gIssueId);
  }
  ...
  function renderCover() {
    ...
    // Update page title for giscus pathname mapping
    document.title = (_story.title || "Story") + " — StoryQuest";
  }

  function loadGiscus(issueId) {
    const container = document.getElementById("giscus-container");
    if (!container) return;

    // Set the term to the issue id so each story gets its own discussion
    container.innerHTML = "";
    const script = document.createElement("script");
    script.src = "https://giscus.app/client.js";
    script.setAttribute("data-repo",            "TheshanBMR/StoryQuest");
